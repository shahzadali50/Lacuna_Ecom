            ...
            <div class="container mx-auto px-2 sm:px-4">
                ...
                <form class="w-full" v-else @submit.prevent="orderGenerate()">
                    ...
                    <Row class="justify-between px-6">
                        ...
                        <div class="mb-4">
                            <label class="block">{{ translations.name || 'Full Name' }} <span class="text-red-500">*</span></label>
                            <a-input v-model:value="form.name" class="mt-2 w-full" :placeholder="translations.name_placeholder" />
                            <div v-if="form.errors.name" class="text-red-500">{{ form.errors.name }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="block">{{ translations.email || 'Email' }} <span class="text-red-500">*</span></label>
                            <a-input type="email" v-model:value="form.email" class="mt-2 w-full" :placeholder="translations.email_placeholder" />
                            <div v-if="form.errors.email" class="text-red-500">{{ form.errors.email }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="block">{{ translations.phone || 'Phone Number' }} <span class="text-red-500">*</span></label>
                            <a-input type="number" v-model:value="form.phone_number" class="mt-2 w-full"
                                :placeholder="translations.phone_placeholder" />
                            <div v-if="form.errors.phone_number" class="text-red-500">{{ form.errors.phone_number }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="block">{{ translations.address || 'Address' }} <span class="text-red-500">*</span></label>
                            <a-textarea v-model:value="form.address" class="mt-2 w-full" :placeholder="translations.address_placeholder"
                                :rows="4" />
                            <div v-if="form.errors.address" class="text-red-500">{{ form.errors.address }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="block">{{ translations.country || 'Country' }} <span class="text-red-500">*</span></label>
                            <a-input v-model:value="form.country" class="mt-2 w-full" :placeholder="translations.country_placeholder" />
                            <div v-if="form.errors.country" class="text-red-500">{{ form.errors.country }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="block">{{ translations.state || 'State' }} <span class="text-red-500">*</span></label>
                            <a-input v-model:value="form.state" class="mt-2 w-full" :placeholder="translations.state_placeholder" />
                            <div v-if="form.errors.state" class="text-red-500">{{ form.errors.state }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="block">{{ translations.city || 'City' }} <span class="text-red-500">*</span></label>
                            <a-input v-model:value="form.city" class="mt-2 w-full" :placeholder="translations.city_placeholder" />
                            <div v-if="form.errors.city" class="text-red-500">{{ form.errors.city }}</div>
                        </div>
                        <div class="mb-4">
                            <label class="block">{{ translations.pcode || 'Postal Code' }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="mb-4">
                            <label class="block">{{ translations.order_notes || 'Order notes (optional)' }}</label>
                        ...
                        <div>
                            ...
                            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-6">{{ translations.check_your_order || 'Check Your Order' }}</h1>
                        </div>
                        <CartItems />
                        ...
                        <div class="flex justify-between mb-4 border-b py-4">
                            <span class="font-medium">{{ translations.shipping_charge || 'Shipping' }}</span>
                            <span class="font-bold text-primary">{{ translations.free_delivery || 'Free Delivery' }}</span>
                        </div>
                        ...
                        <div>
                            ...
                            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-6">{{ translations.payment_method || 'Payment Method' }}</h1>
                            <div class="">
                                <div class="flex items-center">
                                    <a-radio-group v-model:value="form.payment_method" class="ml-2">
                                        <a-radio value="Cash On Delivery">{{ translations.cash_on_delivery || 'Cash On Delivery' }}</a-radio>
                                    </a-radio-group>
                                </div>
                            ...
                            <div>
                                <a-button
                                    html-type="submit"
                                    class="w-full btn-primary mt-5 flex items-center justify-center"
                                    :disabled="form.processing"
                                >
                                    <LoaderCircle v-if="form.processing" class="h-4 w-4 animate-spin mr-2" />
                                    <template #icon v-if="!form.processing"><ShoppingOutlined /></template>
