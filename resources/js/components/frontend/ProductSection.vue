...
<template>
    <section class="py-14">
        <div class="container mx-auto px-2 sm:px-4">
            ...
            <div v-if="props.showFilter === true">
                <a-button size="large" @click="openFilterDrawer">
                    Show Filter
                </a-button>
                <a-drawer title="Filter Products" placement="left" :open="isFilterDrawerVisible"
                    @close="closeFilterDrawer">
                    <!-- Add your filter form or content here -->
                    <div>
                        <p>Filter options go here.</p>
                           <Button size="small" type="primary" aria-label="View all products">
                        <Link :href="route('all.products')" class="text-blue-500 hover:underline mb-4">
                            {{ translations.clear_filters || 'Clear Filters' }}

                        </Link>
                           </Button>
                        <a-collapse v-model:activeKey="activeKey" :bordered="false">
                            <a-collapse-panel key="1" header="Filter by Category">
                              <div  v-for="category in props.categories" :key="category.id"
                              @click="filterByCategory(category.slug)"
                                    class="flex justify-between items-center mb-2">
                                    <p class="font-semibold">{{ category.name }}</p>
                                    <p>{{ category.product_count }} {{ translations.products || 'Products' }}</p>
                                </div>
                            </a-collapse-panel>
